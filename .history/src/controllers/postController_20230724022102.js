   ...
   ////////////////////////////////////////////////////////////
    ...
    updatePostCommentList: async function(req, res) {
        ...
        try {
            console.log("Entered")
            const post = await posts.findOne({num: parseInt(req.body.id)});
            const comment = await comments.find({comment: req.body.comment}).toArray();
            const put_comment = comment.slice(-1);find().limit(5).sort({$natural:-1}) 
            console.log("POST");
            console.log(post);
            console.log("COMMENT");
            console.log(comment);
            ...
            console.log(put_comment.find(comment => comment._id));
            
            const result = await posts.updateOne(post, 
                {
                    $set: {num_comments: post.num_comments + 1},
                    $push: {comments_id: put_comment._id}
                }
            )
